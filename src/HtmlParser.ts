...
import StringQueue from './StringQueue';
...
export class HtmlParser {
  ...
  */
  ...
  private internalParse(parent?: HtmlNode): HtmlNode[] {
    var { q } = this
    var list: HtmlNode[] = [];
    var node: HtmlNode = HtmlNode.empty;
    ...
    while (q.any()) {
      ...
      if (q.peek(2) == "</") {
        ...
        else if (q.peek() == '>' && this.options.selfClosingTags.includes(node.tag)) {
          q.dequeue();
          node.selfClosing = true;
          node.setEndIndex(this.index, this.source);
          list.push(node);
  ...
  GetAttributes(): HtmlNodeAttributes {
    ...
    const GetAttribute = (): KeyValuePair<string, string> => {
      ...
      if (q.any()) {
        ...
        if (q.peek() == '\'' || q.peek() == '"') {
          var del = this.q.dequeue();
          var value = this.q.run(del);
          this.q.dequeue();
          return new KeyValuePair<string, string>(name, value);
        }
        else {
          var value = this.q.run(' ', '>', '<', '\'', '"', '=', '`');
          return new KeyValuePair<string, string>(name, value);
        }
      }
